...
Now we will perform 10 nanoseconds of simulations on the system we have created, utilizing colvars to restrict the protein backbone and track the drug position. 
...
- This script creates a Particle-mesh Ewald grid with a python script, but really this is not necessary and can be simplified 
...
Next modify `outputName`, which will give the pattern of all of NAMD's output. For example, the trajectory files will be called `2l0j_alm034_production.dcd` in the `output/` directory.
```diff 
